...
public class QueryPlan {
  ...
  private QueryOperator pushDownWheres(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    ...
    for (int i = 0; i < whereColumnNames.size(); i++) {
      if (i != except) {
        try {
          String columnName = newSource.checkSchemaForColumn(source.getOutputSchema(), whereColumnNames.get(i));
          newSource = new WhereOperator(newSource, columnName, whereOperators.get(i), whereDataTypes.get(i));
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    int minWhereIdx = -1;
    IndexScanOperator iso = null;
    for (int i = 0; i < whereIndices.size(); i++) {
      iso = new IndexScanOperator(this.transaction, table, whereColumnNames.get(whereIndices.get(i)),
              whereOperators.get(whereIndices.get(i)), whereDataTypes.get(whereIndices.get(i)));
      if (iso.estimateIOCost() < minCost) {
        minWhereIdx = i;
        minCost = iso.estimateIOCost();
        minOp = iso;
      }
    }
  ...
  private QueryOperator minCostJoinType(QueryOperator leftOp,
    ...
    for (JoinOperator.JoinType jt : JoinOperator.JoinType.values()) {
      switch (jt) {
        case SNLJ:
          joinOperator = new SNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
          break;
        case PNLJ:
          joinOperator = new PNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
          break;
        case BNLJ:
          joinOperator = new BNLJOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
          break;
        case GRACEHASH:
          joinOperator = new GraceHashOperator(leftOp, rightOp, leftColumn, rightColumn, this.transaction);
          break;
      }
      if ((minCost == -1) || (joinOperator.estimateIOCost() < minCost)) {
        minCost = joinOperator.estimateIOCost();
        minOp = joinOperator;
      }

      switch (jt) {
        case SNLJ:
          joinOperator = new SNLJOperator(rightOp, leftOp, rightColumn, leftColumn, this.transaction);
          break;
        case PNLJ:
          joinOperator = new PNLJOperator(rightOp, leftOp, rightColumn, leftColumn, this.transaction);
          break;
        case BNLJ:
          joinOperator = new BNLJOperator(rightOp, leftOp, rightColumn, leftColumn, this.transaction);
          break;
        case GRACEHASH:
          joinOperator = new GraceHashOperator(rightOp, leftOp, rightColumn, leftColumn, this.transaction);
          break;
      }
      if ((minCost == -1) || (joinOperator.estimateIOCost() < minCost)) {
        minCost = joinOperator.estimateIOCost();
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    String rightJoinTableName;

    for (Set prevMapTables : prevMap.keySet()) {
      int index = 0;
      while (index < this.joinTableNames.size()) {
        leftJoinColumnName = this.joinLeftColumnNames.get(index);
        rightJoinColumnName = this.joinRightColumnNames.get(index);
        rightJoinTableName = this.joinTableNames.get(index);

        Set<String> tableKey = new HashSet<String>();
        tableKey.add(rightJoinTableName);
        if (pass1Map.containsKey(tableKey) && !prevMapTables.contains(rightJoinTableName)) {
          QueryOperator leftOperator = prevMap.get(prevMapTables);
          QueryOperator rightOperator = pass1Map.get(tableKey);
          QueryOperator optimalJoinOperator = minCostJoinType(leftOperator, rightOperator, leftJoinColumnName, rightJoinColumnName);

          prevMapTables.add(rightJoinTableName);
          map.put(prevMapTables, optimalJoinOperator);
        }
        index += 1;
      }
    }
