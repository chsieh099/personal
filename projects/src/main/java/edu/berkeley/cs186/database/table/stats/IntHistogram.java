...
public class IntHistogram implements Histogram<Integer>{
  ...
  public float computeReductionFactor(PredicateOperator predicate,
    ...
    int numDistinct = getNumDistinct();

    float equalsRF =  1 / (float) numDistinct;
    float greaterThanRF = ((float) (maxValue - value.getInt())) / (maxValue - minValue);
    float lessThanRF = ((float) (value.getInt() - minValue)) / (maxValue - minValue);

    switch (predicate) {
      case EQUALS:
        reductionFactor = equalsRF;
        break;
      case LESS_THAN:
        reductionFactor = lessThanRF;
        break;
      case LESS_THAN_EQUALS:
        reductionFactor = equalsRF + lessThanRF;
        break;
      case GREATER_THAN:
        reductionFactor = greaterThanRF;
        break;
      case GREATER_THAN_EQUALS:
        reductionFactor = greaterThanRF + equalsRF;
        break;
      case NOT_EQUALS:
        reductionFactor = 1 - equalsRF;
        break;
    }
